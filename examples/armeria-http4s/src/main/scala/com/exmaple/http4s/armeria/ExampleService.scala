...
import com.example.http4s.armeria.html
import fs2.Stream
import org.http4s._
import org.http4s.circe._
import io.circe.Json
import org.http4s.dsl.{Http4sDsl, impl}
...
import org.http4s.server.armeria.ServiceRequestContexts
import org.http4s.server.middleware.authentication.BasicAuth
import org.http4s.server.middleware.authentication.BasicAuth.BasicAuthenticator
import org.http4s.twirl._
import scala.concurrent.duration._

// Forked form http4s ExampleService
class ExampleService[F[_]](blocker: Blocker)(implicit F: Effect[F], cs: ContextShift[F])
  ...
  def rootRoutes(implicit timer: Timer[F]): HttpRoutes[F] =
    HttpRoutes.of[F] {
      ...
      case req @ GET -> Root / "protocol" =>
        req.attributes.lookup(ServiceRequestContexts.Key).fold(Ok("unknown")) { ctx =>
          Ok(ctx.sessionProtocol().uriText().toUpperCase())
        }

      case GET -> impl.Root / "ping" =>
        // EntityEncoder allows for easy conversion of types to a response body
        Ok("pong")

      case GET -> impl.Root / "streaming" =>
